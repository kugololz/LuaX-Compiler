...
public:
    ...
    TokenValue next_token() {
        ...
        if (current == 'e' && src.substr(index, 6) == "elseif") {
    index += 6;
...
if (current == 'r' && src.substr(index, 6) == "repeat") {
    index += 6;
    return {Token::REPEAT, "repeat"};
}

if (current == 'u' && src.substr(index, 5) == "until") {
    index += 5;
...
void execute_instructions(const std::vector<Instruction>& instructions, SymbolTable& symbolTable) {
    ...
    while (pc < instructions.size()) {
        ...
        switch (instr.op) {
            ...
            case OpCode::CONCAT_OP: {
    if (stack.size() < 2) {
        std::cerr << "Error: No hay suficientes valores en la pila para CONCAT_OP." << std::endl;
        has_error = true;
        break;
    }
    Value b = stack.back(); stack.pop_back();
    ...
    if (a.type == DataType::STRING && b.type == DataType::STRING) {
        result.string_value = a.string_value + b.string_value;
    } else {
        std::cerr << "Error: Tipos incompatibles para CONCAT_OP." << std::endl;
        has_error=true;
...
void parse() {
    while (current_token.type != Token::END_OF_FILE && !has_error) {
        parse_statement();
    }

    if (has_error) {
        std::cerr << "Se encontraron errores durante la compilación. No se generará código intermedio." << std::endl;
        return;
    }
...
void parse_statement() {
    ...
    } else if (current_token.type == Token::REPEAT) {
        parse_repeat_until_statement();
    } else {
...
void parse_print_statement() {
    ...
    if (has_parentheses) {
        if (current_token.type == Token::RPAREN) {
            ...
            std::cerr << "Error: Se esperaba ')' después de la expresión en 'print'." << std::endl;
            has_error=true;
        }
    }
 ...
 void parse_if_statement() {
    ...
    if (current_token.type == Token::THEN) {
        ...
        while (current_token.type != Token::ELSE && current_token.type != Token::ELSEIF && current_token.type != Token::END) {
            parse_statement();
        }
        ...
        // Manejar cláusulas 'elseif'
        while (current_token.type == Token::ELSEIF) {
            current_token = lexer.next_token(); // Consumir 'elseif'
            ...
            size_t jmp_elseif_false_index = instructions.size() - 1;

            if (current_token.type == Token::THEN) {
                current_token = lexer.next_token(); // Consumir 'then'

                // Parsear el bloque 'elseif'
                while (current_token.type != Token::ELSE && current_token.type != Token::ELSEIF && current_token.type != Token::END) {
                    parse_statement();
                }

                // Instrucción para saltar al final del 'if'
                instructions.push_back(Instruction(OpCode::JMP, 0)); // Dirección de salto a actualizar
                size_t jmp_elseif_end_index = instructions.size() - 1;

                // Actualizar la dirección de salto del 'JMP_IF_FALSE' anterior
                size_t elseif_block_start = instructions.size();
                instructions[jmp_elseif_false_index].operand_int = elseif_block_start;

                // Agregar el índice de salto al final del 'if' para este bloque 'elseif'
                instructions[jmp_end_index].operand_int = instructions.size();
                jmp_end_index = jmp_elseif_end_index;

            } else {
                std::cerr << "Error: Se esperaba 'then' después de 'elseif'." << std::endl;
                has_error=true;
            }
        }
...
void parse_repeat_until_statement() {
    current_token = lexer.next_token(); // Consumir 'repeat'

    size_t loop_start_index = instructions.size(); // Marca el inicio del bucle

    // Parsear el cuerpo del bucle
    while (current_token.type != Token::UNTIL && current_token.type != Token::END_OF_FILE) {
        parse_statement();
    }

    if (current_token.type == Token::UNTIL) {
        current_token = lexer.next_token(); // Consumir 'until'

        // Parsear la condición
        parse_expression();

        // Instrucción para salto condicional
        instructions.push_back(Instruction(OpCode::JMP_IF_FALSE, loop_start_index));
    } else {
        std::cerr << "Error: Se esperaba 'until' después del cuerpo del ciclo 'repeat'." << std::endl;
        has_error=true;
...
void parse_expression() {
    ...
    while (current_token.type == Token::CONCAT) {
        Token op = current_token.type;
        current_token = lexer.next_token();
        parse_relational_expression();
        if (op == Token::CONCAT) {
            instructions.push_back(Instruction(OpCode::CONCAT_OP));
        }
    }
}
